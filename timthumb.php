...
define ('MAX_FILE_SIZE', 6000000);			// file size limit to prevent possible DOS attacks (roughly 1.5 megabytes)
...
if (file_exists ($src)) {
	...
	if ($zoom_crop > 0) {
		...
		// force crop with no scaling
		if ($zoom_crop == 4) {
			$src_w = $new_width;
			$src_h = $new_height;
			$src_x = round( ($width / 2) - ($new_width / 2) );
			$src_y = round( ($height / 2) - ($new_height / 2) );
		}

		// positional cropping!
		if (preg_match('/^\d{1,'.strlen(MAX_WIDTH).'}x\d{1,'.strlen(MAX_HEIGHT).'}$/', $align ) == true)
		{
			list( $src_x, $src_y ) = split("x", $align); 
		} else {
