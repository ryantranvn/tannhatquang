  ...
  * @return mixed */
    ...
    protected function checkUploadedFile(array $aFile=null) {
        ...
        elseif ( 
            ...
            filesize($file['tmp_name']) > $config['maxFileSize']
        )
            return $this->label("File size is too large.");

        // DIMENSION
        elseif ( $width > $config['maxWidth'] || $height > $config['maxHeight']
        )
            return $this->label("Image size is too large.");
